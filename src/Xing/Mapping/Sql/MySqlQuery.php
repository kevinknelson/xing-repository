...
namespace Xing\Mapping\Sql {
    ...
    class MySqlQuery extends APropertiedObject implements ISqlQuery {
        ...
        public function andWhereBetween( $sql, $param1, $param2 ) {
            $sql        = str_replace("{0}", is_null($param1) ? 'NULL' : $this->Pdo->quote((string)$param1), $sql);
            $sql        = str_replace("{1}", is_null($param2) ? 'NULL' : $this->Pdo->quote((string)$param2), $sql);
        ...
        private function formatString( $sql, $params ) {
        ...
        public function saveArray( $table, $primaryKey, $arr ) {
            ...
            foreach( $arr AS $key => $value ) {
                $quotedValue    = is_null($value) ? 'NULL' : (
                                    $value instanceof PreParsedField
                                    ? $value->getParsedString( function($value) use ($pdo) { return $pdo->quote((string)$value); } )
                                    : ( $pdo->quote((string) $value))
                                );
                if( $key == $primaryKey ) {
                    if( !empty($value) ) {
                        $command    = 'UPDATE ';
                        $where      = " WHERE {$key}={$quotedValue}";
                    }
                }
                else {
                    $cols[]   = " {$key}={$quotedValue}";
                }
            }
